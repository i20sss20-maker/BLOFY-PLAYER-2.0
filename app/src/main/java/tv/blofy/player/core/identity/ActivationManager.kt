...
class ActivationManager(
    ...
    suspend fun ensureIdentity(): ActivationEntity {
        ...
        if (existing != null) {
            // The server-known identity is canonical during an app update.
            // Never replace it merely because a newer build uses a different
            // local identity storage scheme; doing so detaches activation and
            // portal playlists from the device the customer already uses.
            val preservedDeviceId = DeviceIdentity.adoptExistingDeviceId(context, existing.deviceId)
            val reconciledCode = DeviceIdentity.reconcileExistingActivationCode(context, existing.activationCode)

            if (preservedDeviceId == existing.deviceId && reconciledCode == existing.activationCode) {
                return existing
            }

            val preserved = existing.copy(
                deviceId = preservedDeviceId,
                activationCode = reconciledCode
            )
            if (preserved.deviceId == existing.deviceId) {
                dao.upsertActivation(preserved)
            } else {
                // Only malformed/unsupported legacy IDs can reach this fallback.
                // Normal valid BLOFY IDs are preserved exactly.
                dao.replaceActivation(
                    preserved.copy(
                        activated = false,
                        expiresAt = null,
