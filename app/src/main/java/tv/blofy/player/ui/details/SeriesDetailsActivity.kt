...
class SeriesDetailsActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        body.addView(
            posterCard,
            LinearLayout.LayoutParams(dp(254), dp(372)).apply {
                marginEnd = dp(34)
            }
        )
        ...
        lifecycleScope.launch {
            ...
            val stream = dao.stream(contentKey) ?: run {
                finish()
                return@launch
            }
            val enrichment = withContext(Dispatchers.IO) {
                val metadata = CinematicMetadataRepository.series(
                    applicationContext,
                    stream.name,
                    stream.year
                )
                val similar = if (metadata == null) {
                    emptyList()
                } else {
                    val titles = CinematicMetadataRepository.recommendations(metadata)
                    SimilarStrip.match(
                        titles,
                        dao.allStreamsForProvider(providerId),
                        "series"
            ...
            ArtworkLoader.loadPriority(
                backdrop,
                listOf(metadata?.backdropUrl, stream.backdrop, stream.icon)
            )
            ArtworkLoader.loadPriority(
                poster,
                listOf(metadata?.posterUrl, stream.icon, stream.backdrop)
            )
            ...
            metadata?.logoUrl?.takeIf(String::isNotBlank)?.let { logoUrl ->
                ...
                panel.addView(
                    logo,
                    LinearLayout.LayoutParams(dp(390), dp(86)).apply {
                        gravity = Gravity.RIGHT
                        topMargin = dp(4)
                    }
                )
            ...
            panel.addView(TextView(this@SeriesDetailsActivity).apply {
                text = buildList {
                    add("مسلسل")
                    (metadata?.releaseDate?.take(4) ?: stream.year)
                        ?.takeIf(String::isNotBlank)
                        ?.let(::add)
                    if (seasons > 0) add("$seasons موسم")
                    if (allEpisodes.isNotEmpty()) add("${allEpisodes.size} حلقة")
                    metadata?.runtimeMinutes?.takeIf { it > 0 }?.let {
                        add("$it دقيقة")
                    }
                    metadata?.rating?.let {
                        add("TMDb %.1f/10".format(java.util.Locale.US, it))
                    } ?: stream.rating?.takeIf(String::isNotBlank)?.let {
                        add("★ $it")
                    }
                    (metadata?.genres?.firstOrNull()
                        ?: stream.genre?.substringBefore(','))
                        ?.takeIf(String::isNotBlank)
                        ?.let(::add)
                }.joinToString("   •   ")
                textSize = 13.5f
            ...
            panel.addView(TextView(this@SeriesDetailsActivity).apply {
                text = metadata?.overview?.takeIf(String::isNotBlank)
                    ...
                    ?: "اختر الموسم والحلقة لبدء المشاهدة."
                textSize = 15f
                typeface = BlofyTvDesign.BodyTypeface
                maxLines = 4
            ...
            if (!metadata?.crew.isNullOrEmpty()) {
                panel.addView(TextView(this@SeriesDetailsActivity).apply {
                    text = metadata!!.crew.joinToString("   •   ") {
                        "${it.job}: ${it.name}"
                    }
                    textSize = 11.5f
            ...
            resume?.let { currentResume ->
                val progress = if (currentResume.durationMs > 0) {
                    ((currentResume.positionMs * 100) / currentResume.durationMs)
                        .toInt()
                        .coerceIn(1, 99)
                } else {
                    0
                ...
                panel.addView(TextView(this@SeriesDetailsActivity).apply {
                    text = "متابعة الموسم ${currentResume.episode.season}   •   " +
                        "الحلقة ${currentResume.episode.episode}" +
            ...
            resume?.let { currentResume ->
                val resumeButton = actionButton("▶  استئناف الحلقة", true) {
                    launchEpisode(
                        provider,
                        currentResume.episode,
                        currentResume.positionMs
                    )
                }
                primary = resumeButton
                actions.addView(
                    resumeButton,
                    LinearLayout.LayoutParams(dp(205), dp(56)).apply {
                        marginStart = dp(8)
                    }
                )
                actions.addView(
                    actionButton("↺  من البداية") {
                        launchEpisode(provider, currentResume.episode, 0L)
                    },
                    LinearLayout.LayoutParams(dp(145), dp(56)).apply {
                        marginStart = dp(8)
                    }
                )
            }
            ...
            val episodes = actionButton(
                ...
                startActivity(
                    Intent(
                        ...
                        putExtra(
                            EpisodesActivity.EXTRA_SERIES_ART,
                            metadata?.backdropUrl ?: stream.backdrop ?: stream.icon
                        )
                    }
                )
            }
            ...
            actions.addView(
                episodes,
                LinearLayout.LayoutParams(dp(215), dp(56)).apply {
                    marginStart = dp(8)
                }
            )
            ...
            metadata?.trailerUrl?.takeIf(String::isNotBlank)?.let { trailerUrl ->
                actions.addView(
                    actionButton("▷  الإعلان") { openExternal(trailerUrl) },
                    LinearLayout.LayoutParams(dp(142), dp(56)).apply {
                        marginStart = dp(8)
                    }
                )
            }
            ...
            actions.addView(
                favoriteButton,
                LinearLayout.LayoutParams(dp(150), dp(56))
            )
            ...
            if (!metadata?.cast.isNullOrEmpty()) {
                ...
                panel.addView(
                    CastStrip.build(this@SeriesDetailsActivity, metadata!!.cast),
            ...
            if (similar.isNotEmpty()) {
                ...
                panel.addView(
                    SimilarStrip.build(this@SeriesDetailsActivity, similar) { selected ->
                        startActivity(
                            Intent(
                                this@SeriesDetailsActivity,
                                SeriesDetailsActivity::class.java
                            ).apply {
    ...
    private fun launchEpisode(
        ...
        startActivity(Intent(this, PlayerActivity::class.java).apply {
            ...
            putExtra(
                PlayerActivity.EXTRA_FALLBACK_URL,
                ContentUrlResolver.directFallback(episode)
            )
    ...
    private fun openExternal(url: String) {
        runCatching {
            ...
            Toast.makeText(
                this,
                "تعذر فتح الإعلان على هذا الجهاز",
                Toast.LENGTH_SHORT
            ).show()
        }
    }
    ...
    private fun actionButton(
        ...
        setOnFocusChangeListener { view, focused ->
            ...
            view.animate().cancel()
            view.animate()
                .scaleX(if (focused) 1.022f else 1f)
                .scaleY(if (focused) 1.022f else 1f)
                .translationZ(
                    if (focused) dp(9).toFloat() else dp(2).toFloat()
                )
                .setDuration(65)
                .start()
        }
