...
class QuickMenuActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        addAction(panel, "⌕", "البحث", "ابحث في القنوات والأفلام والمسلسلات والممثلين", Intent(this, SearchActivity::class.java))
        addAction(panel, "▶", "متابعة المشاهدة", "ارجع مباشرة لآخر نقطة", Intent(this, LibraryActivity::class.java).putExtra(LibraryActivity.EXTRA_MODE, LibraryActivity.MODE_CONTINUE))
        addAction(panel, "★", "المفضلة", "كل اختياراتك في مكان واحد", Intent(this, LibraryActivity::class.java).putExtra(LibraryActivity.EXTRA_MODE, LibraryActivity.MODE_FAVORITES))
        addAction(panel, "◉", "آخر القنوات", "ارجع لآخر بث شاهدته", Intent(this, RecentChannelsActivity::class.java))
        addAction(panel, "✦", "مختارات BLOFY", "الأعلى تقييمًا و4K وعربي", Intent(this, SmartCollectionsActivity::class.java).putExtra(SmartCollectionsActivity.EXTRA_MODE, SmartCollectionsActivity.MODE_TOP_RATED))
        addAction(panel, "⚙", "الإعدادات", "المشغل والتحديث والنظام", Intent(this, SettingsActivity::class.java))
    ...
    private fun addAction(parent: LinearLayout, icon: String, title: String, subtitle: String, intent: Intent) {
        val row = LinearLayout(this).apply {
            ...
            addView(iconView, LinearLayout.LayoutParams(dp(48), dp(48)).apply { marginStart = dp(12) })
