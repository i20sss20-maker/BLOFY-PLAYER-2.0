...
/** Dedicated TV catalog with keyset paging for very large providers. */
class PosterCatalogActivity : AppCompatActivity() {
    ...
    private fun loadStreams(categoryId: String?, immediate: Boolean) {
        ...
        if (cached != null && cached.items.isNotEmpty()) {
            ...
            updateCount()
            val focusKey = cached.focusedKey ?: savedPosterKey()
            focusKey?.let(::restorePosterKeyIfLoaded)
            if (loadedItems.size < totalItems && loadedItems.size <= PREFETCH_THRESHOLD) loadNextPage()
            return
        }
    ...
    private fun loadNextPage(reset: Boolean = false) {
        ...
        pageJob = lifecycleScope.launch {
            ...
            val result = withContext(Dispatchers.IO) {
                ...
                val nextRowId = page.lastOrNull()?.let { dao.streamRowId(it.key) } ?: cursor
                Triple(total, page, nextRowId)
            }
    ...
    private fun saveMemorySnapshot() {
        ...
        CatalogPageMemory.put(memoryKey(), loadedItems, totalItems, lastRowId, savedPosterKey())
    }

    private fun memoryKey(): String = "$providerId:$kind:${selectedCategoryId ?: ALL_CATEGORY_ID}"

    private fun restoreSavedPosterIfVisible() {
        savedPosterKey()?.let(::restorePosterKeyIfLoaded)
    }
