...
class SearchActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        hint = TextView(this).apply {
            text = "القنوات، الأفلام والمسلسلات تظهر من أول حرف"
            textSize = 13f
    ...
    private fun runSearch(query: String, moveFocus: Boolean) {
        ...
        lifecycleScope.launch {
            ...
            val items = withContext(Dispatchers.IO) { ContentRepository(dao).search(provider.id, q) }
            if (input.text?.toString()?.trim() != q) return@launch
            results.removeAllViews()
            hint.text = "${items.size.coerceAtMost(100)} نتيجة"
            if (items.isEmpty()) {
                showMessage("ما لقينا نتائج مطابقة")
                return@launch
            }
    ...
    private fun resultCard(stream: StreamEntity, open: () -> Unit): LinearLayout {
        ...
        return LinearLayout(this).apply {
            ...
            val copy = LinearLayout(this@SearchActivity).apply {
                orientation = LinearLayout.VERTICAL
            ...
            val badge = TextView(this@SearchActivity).apply {
                ...
                background = GradientDrawable().apply {
                    ...
                    setColor(0x66382252)
                    setStroke(dp(1), 0x995F3D82.toInt())
                }
            }
    ...
    private fun searchField(focused: Boolean) = GradientDrawable(
        ...
        setStroke(if (focused) dp(2) else dp(1), if (focused) BlofyTvDesign.PurpleBright else 0xFF513D67.toInt())
    }
    ...
    private fun rowBackground(focused: Boolean) = GradientDrawable(
        ...
        setStroke(if (focused) dp(2) else dp(1), if (focused) BlofyTvDesign.PurpleBright else 0xFF49375E.toInt())
    }
