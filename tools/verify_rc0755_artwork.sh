...
for method in firstAttemptRecoversWithoutShowingAnError favoritesOpenQuicklyInTwoHundredThousandRowsDuringWrites; do
  adb shell am instrument -w -r -e class "tv.blofy.player.ui.CatalogRecoveryDeviceTest#$method" \
    tv.blofy.player.v2.test/androidx.test.runner.AndroidJUnitRunner | tee "artwork-evidence/$method.txt"
  grep -Fq 'OK (1 test)' "artwork-evidence/$method.txt"
  if grep -Eq 'FAILURES|INSTRUMENTATION_FAILED|Process crashed|INSTRUMENTATION_STATUS_CODE: -[1234]' "artwork-evidence/$method.txt"; then exit 1; fi
  adb shell am force-stop tv.blofy.player.v2
...
test -f artwork-evidence/artwork-qa/first-attempt-result.txt
test -f artwork-evidence/artwork-qa/favorites-200k-result.txt
