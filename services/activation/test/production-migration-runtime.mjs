...
async function runMigration(extraEnv={}) {
  const child = spawn(process.execPath, ['src/production-migration.mjs'], {
    ...
    env: { ...process.env, PGSSLMODE:'disable', SOURCE_DATABASE_URL:sourceUrl, DATABASE_URL:targetUrl,
      SOURCE_BLOFY_PLAYLIST_ENCRYPTION_KEY:key, BLOFY_PLAYLIST_ENCRYPTION_KEY:key, ...extraEnv },
...
try {
  ...
  try {
    ...
    const sourceDevice='BLOFY-MIGRATION-SOURCE';
    await source.query(`INSERT INTO devices(device_id,activation_code,status,expires_at,last_app_version,last_platform)
      VALUES($1,'h1:synthetic-proof','active',NOW()+INTERVAL '90 days','2.0.0-test','android')`,[sourceDevice]);
    ...
    await source.query(`INSERT INTO device_playlists(id,device_id,name,provider_type,base_url_enc,username_enc,password_enc,active,revision)
      VALUES($1,$2,'قائمة اختبار','xtream','enc-base','enc-user','enc-pass',TRUE,3)`,[playlistId,sourceDevice]);
    ...
    assert.equal(result.code,0,result.stderr);const dry=JSON.parse(result.stdout);assert.equal(dry.mode,'dry-run');assert.equal(dry.keyCompatible,true);
    ...
    assert.equal(result.code,0,result.stderr);const applied=JSON.parse(result.stdout);assert.equal(applied.mode,'applied');assert.equal(applied.keyCompatible,true);
    ...
    result=await runMigration({SOURCE_BLOFY_PLAYLIST_ENCRYPTION_KEY:'b'.repeat(64),BLOFY_MIGRATION_APPLY:'YES_COPY_BLOFY_PRODUCTION_TO_AZURE',BLOFY_MIGRATION_CONFIRM_SOURCE:sourceName,BLOFY_MIGRATION_CONFIRM_TARGET:targetName});
    assert.equal(result.code,1);assert.match(result.stderr,/migration_encryption_key_mismatch/);
    assert.equal((await target.query('SELECT COUNT(*)::int AS n FROM devices')).rows[0].n,1,'failed guarded run leaves target intact');
    console.log('PASS: migration dry-run, key guard, target backup, canonical copy, count verification, sequence repair and Azure release-store preservation.');
