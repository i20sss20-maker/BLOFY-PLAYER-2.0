...
try{
 ...
 const request=async(path,method='GET',body,override={})=>{const target=new URL(base+path);if(target.pathname==='/api/v1/admin/device-insights'&&method==='GET')target.searchParams.set('format','manage');const response=await fetch(target,{method,headers:{...headers,...override},body:body===undefined?undefined:JSON.stringify(body)});return {status:response.status,headers:response.headers,data:await response.json()};};
 ...
 assert.equal((await request(root,'DELETE')).status,405);
