...
import { APPROVED_RC0745 as release, RC0745_PUBLICATION_ACTION as action, publishApprovedRc0745 as publish } from '../src/approved-release-rc0745.mjs';

const previous = { id:'11111111-1111-4111-8111-111111111111', channel:'testing', version_code:2000055,
  version_name:'2.0.0-rc07.44', min_supported_version_code:1,
  download_url:'https://github.com/i20sss20-maker/BLOFY-PLAYER-2.0/releases/download/v2.0.0-rc07.44/BLOFY-PLAYER-2.0-rc07.44-signed.apk' };
...
test('one approved publication preserves the previous row and is optional',async()=>{
  ...
  assert.equal(insert.args[2],2000056);assert.equal(insert.args[6],1);
...
test('changed, missing or newer primary is recorded without overwriting it',async()=>{
  for(const current of [null,{...previous,version_code:2000057},{...previous,download_url:'https://example.invalid/other.apk'}]){
...
test('a conflicting version-56 row is not silently rewritten or published',async()=>{
  const f=fake({target:{...previous,id:crypto.randomUUID(),version_code:2000056}});
...
async function withCatalog(fn) {
  ...
  const schema='rc0745_'+crypto.randomUUID().replaceAll('-','');
...
test('PostgreSQL: concurrent first-use publication, optional metadata and subsequent admin rollback',{skip:!hasDatabase},async()=>{
  await withCatalog(async({pool,catalog,run,createReleaseCatalog})=>{
    ...
    assert.equal((await catalog.primary()).versionCode,2000056);assert.equal((await catalog.primary()).minSupportedVersionCode,1);
    const old=snapshot.items.find(x=>x.versionCode===2000055);
    ...
    assert.equal((await createReleaseCatalog(pool).primary()).versionCode,2000055);
...
test('PostgreSQL: failed audit rolls back both selection and inserted release',{skip:!hasDatabase},async()=>{
  await withCatalog(async({pool,catalog,run})=>{
    ...
    assert.equal((await catalog.primary()).versionCode,2000055);assert.equal((await catalog.list()).items.length,1);
