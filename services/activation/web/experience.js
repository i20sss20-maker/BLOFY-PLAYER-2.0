...
const labels = {active:'ساري',trial:'تجربة',pending:'تسجيل غير مكتمل',expired:'منتهٍ',blocked:'موقوف',open:'مفتوح',resolved:'تم الحل',testing:'تجريبي',stable:'معتمد'};
...
if(page==='admin'){
  ...
  const expiry=(value,status)=>status==='pending'?'لم تبدأ التجربة':value?date(value):status==='active'?'مدى الحياة':'—';
  ...
  async function overview(){const [data,tickets]=await Promise.all([request('overview'),request('tickets')]);$('overview').replaceChildren();for(const [key,label]of [['total','أجهزة مسجّلة'],['active','تفعيل ساري'],['pending','تسجيل غير مكتمل'],['expired','تفعيل منتهٍ'],['support','طلبات دعم']]){const card=node('div',null,'metric');card.append(node('strong',Number(data[key]||0).toLocaleString('ar-SA')),node('span',label));$('overview').append(card);}
  ...
  async function deviceDetails(deviceId,current){extra.replaceChildren(node('p','جارٍ قراءة تفاصيل الجهاز…','caption'));
    try{const data=await api('/api/v1/admin/device-insights/'+encodeURIComponent(deviceId));if(current!==generation||selected!==deviceId)return;
      ...
      if(data.registrationNote)extra.prepend(node('p',data.registrationNote,'notice'));
      extra.append(node('p','إحصاء التحميل متاح أعلى اللوحة، ولا يمكن ربط كل تنزيل بجهاز محدد. حفظ القائمة بالموقع لا يثبت وصولها للتطبيق.','caption'));
  ...
  async function openRecord(deviceId){selected=deviceId;const current=++generation;setStatus('admin-status','جارٍ فتح السجل…');try{
    ...
    $('record-summary').dataset.status=data.status;$('record-summary').dataset.lifetime=String(data.status==='active'&&data.expiresAt==null);
