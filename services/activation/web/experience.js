...
if(page==='admin'){
  ...
  async function customers(){if(window.BlofyDeviceAdmin)return window.BlofyDeviceAdmin.customers(openRecord);const current=++customerGeneration;setStatus('admin-status','جارٍ تحديث الأجهزة…');try{const query=new FormData($('customer-search')).get('q')||'';const data=await api('/api/v1/admin/users?limit=200&q='+encodeURIComponent(query));if(current!==customerGeneration)return;$('customer-rows').replaceChildren();
  ...
  async function openRecord(deviceId){selected=deviceId;const current=++generation;setStatus('admin-status','جارٍ فتح السجل…');try{
    ...
    if(window.BlofyDeviceAdmin)window.BlofyDeviceAdmin.record(deviceId,()=>current===generation,()=>{openRecord(deviceId);customers();overview().catch(()=>{});});
