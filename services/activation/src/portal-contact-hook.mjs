...
import { normalizePortalDeviceId } from './portal-pending-auth.mjs';
...
async function authorize(req, body) {
  ...
  try {
    ...
    const entitled = ['active', 'trial'].includes(normalizedStatus(row));
    const pending = row.trial_registration_pending === true && row.status === 'expired';
    await client.query('COMMIT');
    return entitled || pending ? { deviceId: row.device_id } : null;
  } catch (error) {
    await client.query('ROLLBACK').catch(() => {});
    throw error;
  } finally { client.release(); }
...
async function handleContactApi(req, res, pathname) {
  ...
  try {
    ...
    await database().query(`INSERT INTO device_customers(device_id,customer_phone,source)
      VALUES($1,$2,'portal')
      ON CONFLICT(device_id) DO UPDATE SET customer_phone=EXCLUDED.customer_phone,source='portal',updated_at=NOW()`, [authorized.deviceId, phone]);
