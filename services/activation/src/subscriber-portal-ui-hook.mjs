...
export function injectSubscriberPortalUi(html) {
  ...
  if (!source.includes('</body>') || source.includes('data-blofy-subscriber-ui="4"')) return source;

  const whatsappNumber = JSON.stringify(WHATSAPP_NUMBER);
  const injection = String.raw`
<style data-blofy-subscriber-ui="4">
  ...
  .blofy-renew-options{display:grid;grid-template-columns:1fr 1fr;gap:10px}.blofy-renew-option{min-height:62px;border:1px solid rgba(184,140,255,.18);border-radius:14px;background:#100d18;color:#fff;font-weight:800;cursor:pointer;display:grid;place-items:center;gap:3px}.blofy-renew-option:hover{border-color:#8b37ff;background:#171122}.blofy-renew-option small{color:#9ff1cb;font-size:12px;font-weight:900}
...
(function () {
  ...
  function resolvedDeviceAuth() { rememberDeviceAuth(); return { deviceId: rememberedDeviceId, activationCode: rememberedActivationCode }; }
  function dispatchValue(node, value) { if (!node) return; node.value = value == null ? '' : String(value); node.dispatchEvent(new Event('input', { bubbles: true })); node.dispatchEvent(new Event('change', { bubbles: true })); }
  function installAuthCapture() { ['deviceId', 'activationCode'].forEach(function (id) { var node = qs(id); if (!node || node.dataset.blofyAuthCapture) return; node.dataset.blofyAuthCapture = '1'; node.addEventListener('input', rememberDeviceAuth, true); node.addEventListener('change', rememberDeviceAuth, true); }); rememberDeviceAuth(); }
  function isSubscriberPlaylist(item) { if (!item || !item.baseUrl) return false; try { return new URL(item.baseUrl).pathname.replace(/\/+$/, '') === '/api/v1/subscribers/xtream'; } catch (_) { return false; } }

  function configureProviderOptions() {
    var select = qs('providerType'); if (!select) return;
    Array.from(select.options).forEach(function (option) { if (option.value === 'm3u') option.remove(); });
    var xtream = select.querySelector('option[value="xtream"]'); if (xtream) xtream.textContent = 'Xtream Codes';
    if (!select.querySelector('option[value="blofy"]')) { var option = document.createElement('option'); option.value = 'blofy'; option.textContent = 'مشتركين BLOFY'; select.insertBefore(option, select.firstChild); }
    if (!qs('blofySubscriberHint')) { var hint = document.createElement('div'); hint.id = 'blofySubscriberHint'; hint.style.display = 'none'; hint.textContent = 'أدخل اسم المستخدم وكلمة المرور فقط. عنوان السيرفر محفوظ داخل BLOFY.'; var grid = select.closest('.form-grid'); if (grid) grid.appendChild(hint); }
  }

  function applyMode() {
    configureProviderOptions(); var select = qs('providerType'); if (!select) return;
    var blofy = select.value === 'blofy'; var base = qs('baseUrl'); var name = qs('name'); var user = qs('username'); var pass = qs('password'); var hint = qs('blofySubscriberHint');
    ...
    if (blofy) { if (name && !name.value.trim()) name.value = 'مشتركين BLOFY'; if (base) base.value = ''; if (user) { user.placeholder = 'اسم المستخدم'; user.required = true; } if (pass) { pass.placeholder = 'كلمة المرور'; pass.required = true; } }
    else { editingSubscriberId = null; if (user) user.required = true; if (pass) pass.required = true; }
  }

  function installRenewalUi() {
    var actions = document.querySelector('.dashboard-head .actions'); if (!actions || qs('blofyRenewBtn')) return;
    var button = document.createElement('button'); button.id = 'blofyRenewBtn'; button.type = 'button'; button.textContent = '↻ تجديد الاشتراك'; actions.insertBefore(button, actions.firstChild);
    var modal = document.createElement('div'); modal.id = 'blofyRenewModal'; modal.className = 'blofy-renew-modal hidden';
    ...
    modal.querySelectorAll('[data-plan]').forEach(function (planButton) {
      planButton.addEventListener('click', function () {
        rememberDeviceAuth(); var deviceId = rememberedDeviceId || 'غير معروف'; var plan = planButton.getAttribute('data-plan'); var price = planButton.getAttribute('data-price');
  ...
  function installFormOverrides() {
    if (window.blofySubscriberOverridesV4) return;
    var originalTypeUi = window.typeUi; var originalEdit = window.edit; var originalOpenEditor = window.openEditor; var originalClearEditor = window.clearEditor;
    if (typeof originalTypeUi !== 'function' || typeof originalEdit !== 'function') return; window.blofySubscriberOverridesV4 = true;
    window.typeUi = function () { var node = qs('providerType'); if (!node || node.value === 'xtream') originalTypeUi.apply(this, arguments); applyMode(); };
    var select = qs('providerType'); if (select) { select.onchange = window.typeUi; select.addEventListener('change', function () { requestAnimationFrame(applyMode); }); }
    if (typeof originalClearEditor === 'function') window.clearEditor = function () { editingSubscriberId = null; originalClearEditor.apply(this, arguments); configureProviderOptions(); var selectNode = qs('providerType'); if (selectNode) selectNode.value = 'xtream'; applyMode(); };
    if (typeof originalOpenEditor === 'function') { window.openEditor = function () { editingSubscriberId = null; originalOpenEditor.apply(this, arguments); configureProviderOptions(); applyMode(); }; var newBtn = qs('newBtn'); if (newBtn) newBtn.onclick = window.openEditor; }
    window.edit = function (item) { originalEdit.apply(this, arguments); configureProviderOptions(); if (!isSubscriberPlaylist(item)) { editingSubscriberId = null; applyMode(); return; } editingSubscriberId = item.id; var providerSelect = qs('providerType'); if (providerSelect) providerSelect.value = 'blofy'; dispatchValue(qs('username'), ''); dispatchValue(qs('password'), ''); dispatchValue(qs('baseUrl'), ''); applyMode(); status('اكتب اسم المستخدم وكلمة المرور من جديد لتحديث اشتراك BLOFY.', false); };
  }

  async function createSubscriberSession() {
    var deviceAuth = resolvedDeviceAuth(); var username = (qs('username') && qs('username').value || '').trim(); var password = (qs('password') && qs('password').value || '');
    if (!username || !password) throw new Error('أدخل اسم المستخدم وكلمة المرور'); if (!deviceAuth.deviceId || !deviceAuth.activationCode) throw new Error('بيانات الجهاز غير مكتملة. أعد الدخول إلى البوابة.');
    ...
    if (!response.ok) { var code = payload && payload.error; if (code === 'invalid_subscriber_credentials') throw new Error('أدخل اسم المستخدم وكلمة المرور'); if (code === 'subscriber_login_failed') throw new Error('اسم المستخدم أو كلمة المرور غير صحيحة'); if (code === 'unauthorized_device') throw new Error('الجهاز غير مفعل أو بيانات الربط غير صحيحة'); if (code === 'subscriber_service_unavailable') throw new Error('خدمة مشتركين BLOFY غير متاحة حاليًا'); if (code === 'subscriber_upstream_unavailable') throw new Error('سيرفر المشتركين لا يستجيب حاليًا'); throw new Error('تعذر تسجيل الدخول إلى مشتركين BLOFY'); }
    if (!payload || !payload.baseUrl || !payload.username || !payload.password) throw new Error('تعذر تجهيز بيانات مشترك BLOFY. حاول مرة أخرى.'); return payload;
  }

  async function saveSubscriber() {
    var button = qs('saveBtn'); if (button.dataset.blofyBusy === '1') return; button.dataset.blofyBusy = '1'; button.disabled = true; status('جاري التحقق من اشتراك BLOFY…', false);
    try { var session = await createSubscriberSession(); var authData = resolvedDeviceAuth(); var response = await fetch('/api/v1/portal/playlists', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ deviceId: authData.deviceId, activationCode: authData.activationCode, id: editingSubscriberId || undefined, name: (qs('name') && qs('name').value.trim()) || session.providerName || 'مشتركين BLOFY', providerType: 'xtream', baseUrl: session.baseUrl, username: session.username, password: session.password, active: !qs('active') || qs('active').checked }) }); var payload = {}; try { payload = await response.json(); } catch (_) {} if (!response.ok) throw new Error(payload.error || 'تعذر حفظ اشتراك BLOFY'); status('تم حفظ اشتراك BLOFY بنجاح.', false); editingSubscriberId = null; if (qs('editor')) qs('editor').classList.add('hidden'); if (typeof window.clearEditor === 'function') window.clearEditor(); if (typeof window.load === 'function') await window.load(); else window.location.reload(); }
    finally { button.disabled = false; button.dataset.blofyBusy = '0'; }
  }

  function installSaveInterceptor() { var button = qs('saveBtn'); var select = qs('providerType'); if (!button || !select || button.dataset.blofySubscriberInterceptorV4) return; button.dataset.blofySubscriberInterceptorV4 = '1'; button.addEventListener('click', async function (event) { if (select.value !== 'blofy') return; event.preventDefault(); event.stopImmediatePropagation(); try { await saveSubscriber(); } catch (error) { status(error && error.message ? error.message : 'تعذر الحفظ', true); } }, true); }
...
http.createServer = function patchedPortalCreateServer(listener) {
  ...
  return previousCreateServer(async (req, res) => {
    ...
    const originalWriteHead = res.writeHead.bind(res); const originalEnd = res.end.bind(res); let statusCode = 200; let statusMessage; let headers = {}; let wroteHead = false;
    ...
    res.end = function interceptedEnd(chunk, encoding, callback) { const body = chunk == null ? '' : Buffer.isBuffer(chunk) ? chunk.toString(encoding || 'utf8') : String(chunk); const modified = injectSubscriberPortalUi(body); if (wroteHead) { for (const key of Object.keys(headers)) if (key.toLowerCase() === 'content-length') delete headers[key]; headers['content-length'] = Buffer.byteLength(modified); if (statusMessage) originalWriteHead(statusCode, statusMessage, headers); else originalWriteHead(statusCode, headers); } return originalEnd(modified, 'utf8', callback); };
