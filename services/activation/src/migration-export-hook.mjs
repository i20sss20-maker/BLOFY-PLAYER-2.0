...
async function exportWindow({ requireMigrationContext = true } = {}) {
  ...
  // Fallback is intentionally compile-time, short-lived, and public-key only.
  // It lets a production migration proceed when raw.githubusercontent.com is
  // temporarily unavailable from the Vercel runtime. An expired/blank config
  // still fails closed.
