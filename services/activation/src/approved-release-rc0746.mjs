...
async function publishApprovedRc0747(client, environment = process.env.VERCEL_ENV) {
 ...
 * primary. Once 46 is present, chain the separately approved rc07.47 publication.
 * A later administrator selection still wins permanently.
 */
export async function publishApprovedRc0746(client, environment = process.env.VERCEL_ENV) {
  ...
  if (done.rows.length) return publishApprovedRc0747(client, environment);
  ...
  if (!current || Number(current.version_code) !== 2000056 || current.version_name !== '2.0.0-rc07.45' || current.download_url !== previousUrl) {
    const outcome = await record('skipped-selection-changed');
    if (current && Number(current.version_code) === APPROVED_RC0746.versionCode &&
        current.version_name === APPROVED_RC0746.versionName && current.download_url === APPROVED_RC0746.downloadUrl) {
      return publishApprovedRc0747(client, environment);
    }
    return outcome;
  }
  ...
  await client.query('UPDATE app_release_selection SET primary_id=$1,revision=revision+1 WHERE singleton=TRUE', [target.id]);
  await record('published', target.id);
  return publishApprovedRc0747(client, environment);
}
