...
const { Pool } = pg;
const sourceUrl = String(process.env.BLOFY_MIGRATION_SOURCE_URL || '').trim();
...
async function alreadyApplied(pool, tokenHash) {
  ...
  return Boolean((await pool.query(
    'SELECT 1 FROM blofy_migration_pull_once WHERE token_hash=$1',[tokenHash]
  )).rows[0]);
}
...
async function pullOnce() {
  ...
  try {
    ...
    const result = await importEncryptedMigrationEnvelope(JSON.parse(text));
    ...
    console.log(`BLOFY Railway migration counts: devices=${result.sourceCounts?.devices ?? 'na'} playlists=${result.sourceCounts?.device_playlists ?? 'na'} diagnostics=${result.sourceCounts?.playback_diagnostics ?? 'na'}`);
...
if (result?.skipped) console.log(`BLOFY Railway migration pull skipped: ${result.reason}`);
