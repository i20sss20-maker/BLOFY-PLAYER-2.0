...
export async function inspectRemoteApk(value) {
  ...
  let response;
  try {
    let result = await fetchPublic(requestedUrl, { method: 'HEAD' });
...
function globPattern(pattern) {
  const escaped = String(pattern || '').replace(/[.+^$(){}|[\]\\]/g, '\\export async function closeAppLibrary() {
  await pool.end();
}').replaceAll('*', '.*');
  return new RegExp(`^${escaped}// Managed TV app library for the Azure download center.
import pg from 'pg';
import dns from 'node:dns/promises';
import net from 'node:net';
...
async function latestGitHubAsset(rule) {
  ...
  const matcher = globPattern(rule.asset);
  const asset = Array.isArray(release.assets)
    ? release.assets.find(item => item && matcher.test(String(item.name || '')) && /\.apk$/i.test(String(item.name || '')))
    : null;
