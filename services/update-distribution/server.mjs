...
function renderAppLibrary(apps) {
  ...
  const rows = apps.map(app => {
    ...
    return `<article class="app-row${app.featured ? ' app-featured' : ''}" data-app-row data-category="${escapeHtml(app.category)}" data-tv="${isTv ? '1' : '0'}" data-search="${escapeHtml(searchText)}">
  ...
  return `<section class="apps-section">
    ...
    <div class="app-controls">
      <label class="search-box" for="app-search"><span>⌕</span><input id="app-search" type="search" placeholder="ابحث عن تطبيق..." autocomplete="off" spellcheck="false"></label>
