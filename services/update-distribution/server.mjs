...
function renderAppLibrary(apps, variants = [], rootUrl = '') {
  ...
  const rows = apps.map(app => {
    ...
    const variantButtons = appVariants.length > 1
      ...
      : '';
    return `<article id="app-${escapeHtml(app.slug)}" class="app-row${app.featured ? ' app-featured' : ''}" data-app-row data-slug="${escapeHtml(app.slug)}" data-category="${escapeHtml(app.category)}" data-tv="${isTv ? '1' : '0'}" data-search="${escapeHtml(searchText)}">
      ...
      <div class="app-actions">
        ...
        <button class="app-more" type="button" data-details-toggle aria-expanded="false" aria-controls="${detailsId}">تفاصيل</button>
      </div>
      <div class="app-details" id="${detailsId}" hidden>
        ...
        <div class="stable-download-link"><strong>رابط مباشر للـ Downloader</strong><a href="${escapeHtml(stableDownloadUrl)}">${escapeHtml(stableDownloadUrl)}</a></div>
        ${variantButtons}
      </div>
    </article>`;
  }).join('');
...
(() => {
  ...
  function setRowDetails(row, open, syncUrl = false) {
    ...
    button.textContent = open ? 'إغلاق' : 'تفاصيل';
