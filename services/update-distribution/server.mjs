...
function renderAppLibrary(apps) {
  const categories = [
    ['media', 'مشغلات وميديا', 'تشغيل الفيديو والملفات والمكتبات على الشاشة.'],
    ['files', 'الملفات والنقل', 'نقل الملفات وإدارة التخزين وملفات APK.'],
    ['downloads', 'التنزيل والمتصفحات', 'أدوات للوصول للروابط وتنزيل الملفات من التلفزيون.'],
    ['network', 'أدوات الشبكة', 'قياس السرعة وفحص اتصال الإنترنت والـ Wi-Fi.'],
    ['launcher', 'تخصيص الشاشة', 'واجهات Home بديلة لترتيب التطبيقات على Android TV.']
  ];
  ...
  if (!groups) return '';
  return `<section class="apps-section"><div class="apps-intro"><div><span class="eyebrow">FOR TV & ANDROID BOX</span><h2>تطبيقات مفيدة للشاشات والرسيفرات</h2><p>تطبيقات مجانية مختارة بعناية. BLOFY PLAYER يظل التطبيق الرسمي الأساسي بالأعلى.</p></div><span class="apps-count">${apps.length} تطبيق</span></div>${groups}</section>`;
}
...
server.listen(PORT, '0.0.0.0', () => {
  console.log(`BLOFY preview distribution listening on ${PORT}`);
});
