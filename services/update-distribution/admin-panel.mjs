...
function categoryLabel(category) {
  return ({ media:'مشغلات وميديا', files:'ملفات ونقل', downloads:'تنزيل ومتصفحات', network:'أدوات الشبكة', launcher:'واجهات الشاشة' })[category] || category;
}

function categoryOptions(current) {
  return [
    ['media','مشغلات وميديا'], ['files','ملفات ونقل'], ['downloads','تنزيل ومتصفحات'],
    ['network','أدوات الشبكة'], ['launcher','واجهات الشاشة']
  ].map(([value,label]) => `<option value="${value}"${current === value ? ' selected' : ''}>${label}</option>`).join('');
}

function modeOptions(current) {
  return [['official','المصدر الرسمي'],['direct','تحميل مباشر']].map(([value,label]) =>
