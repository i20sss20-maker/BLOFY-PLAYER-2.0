...
import { listApps, upsertApp, deleteApp, inspectRemoteApk, refreshManagedApps, getAppUpdateState } from './app-library.mjs';
...
export async function renderAdmin(message = '') {
  ...
  const [apps, updateState] = await Promise.all([listApps(true), getAppUpdateState()]);
