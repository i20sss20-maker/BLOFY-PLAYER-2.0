...
import { listApps, upsertApp, deleteApp, inspectRemoteApk, refreshManagedApps, getAppUpdateState, getDownloadStats, getAppHealthState, refreshAppHealth, rollbackApp, listAppAudit } from './app-library.mjs';
...
export async function renderAdmin(message = '') {
  ...
  const [apps, updateState, downloadStats, healthState, auditEntries] = await Promise.all([
    listApps(true),
    getAppUpdateState(),
    getDownloadStats(),
    getAppHealthState(),
  ...
  const auditRows = auditEntries.length
    ? auditEntries.map(entry => {
        ...
        const versionInfo = details.fromVersion || details.toVersion
          ...
          : '';
        const who = entry.actor === 'system' ? 'النظام' : 'admin';
